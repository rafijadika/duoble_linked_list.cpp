...
void DoubleLinkedList::addNode() {
	...
	if (START == NULL || nim <= START->noMhs) {
		if (START != NULL && nim == START->noMhs) {
			cout << "\nDuplicate nuber noot allowed" << endl;
			return;
		}
		newNode->next = START;
		if (START != NULL)
...
bool DoubleLinkedList::deleteNode(int rollNo) {
	...
	void DoubleLinkedList::ascending() {
		if (listEmpty())
			...
			cout << "\nRecords in ascending order to roll number are:" << endl;
	...
	void DoubleLinkedList::hapus() {
		...
		cout << "/nEnter the roll number of student whos record is to be deleted";
		int rollNo;
		cin >> rollNo;
		cout << endl;
		if (DoubleLinkedList::deleteNode(rollNo) == false)
			cout << "Record not found" << endl;
		else cout << "Record with roll number " rollNo << "deleted" << endl;
